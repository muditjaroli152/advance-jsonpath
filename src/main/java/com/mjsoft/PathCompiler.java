...
public class PathCompiler {
    ...
    private static final char BEGIN_BACK_TRAVERSE = '^';
    ...
    private LinkedList<JPToken> compileJPToken() {
        LinkedList<JPToken> jpTokens = readContextJPToken();
        return jpTokens;
    }
    ...
    public static LinkedList<JPToken> compileJPToken(String path, final Predicate... filters) {
        try {
            CharacterIndex ci = new CharacterIndex(path);
            ci.trim();
            ...
            LinkedList<Predicate> filterStack = new LinkedList<Predicate>(asList(filters));
            LinkedList<JPToken> jpTokens = new PathCompiler(ci, filterStack).compileJPToken();
            return jpTokens;
        } catch (Exception e) {
            InvalidPathException ipe;
            if (e instanceof InvalidPathException) {
    ...
    private RootPathToken readContextToken() {
        ...
        LinkedList<JPToken> jpList = new LinkedList<>();
        jpList.add(new JPToken(TokenType.ROOT,"$"));
        readNextToken(appender,jpList);

        return pathToken;
    }

    private LinkedList<JPToken> readContextJPToken() {
    ...
    //
    //
    private boolean readNextToken(PathTokenAppender appender, LinkedList<JPToken> jpList) {

        char c = path.currentChar();

        switch (c) {
            case OPEN_SQUARE_BRACKET:
                return readBracketPropertyToken(appender,jpList) ||
                        readArrayToken(appender,jpList) ||
                        readWildCardToken(appender,jpList) ||
                        readFilterToken(appender,jpList) ||
                        readPlaceholderToken(appender,jpList) ||
                        readBackTraverseToken(appender,jpList) ||
                        fail("Could not parse token starting at position " + path.position() + ". Expected ?, ', 0-9, * ");
            case PERIOD:
                return readDotToken(appender,jpList) ||
                        fail("Could not parse token starting at position " + path.position());
            case WILDCARD:
                return readWildCardToken(appender,jpList) ||
                        fail("Could not parse token starting at position " + path.position());
            default:
                return readPropertyOrFunctionToken(appender,jpList) ||
                        fail("Could not parse token starting at position " + path.position());
        }
    }

    private boolean readBackTraverseToken(PathTokenAppender appender, LinkedList<JPToken> jpList) {
        if (!path.currentCharIs(OPEN_SQUARE_BRACKET) && !path.nextSignificantCharIs(BEGIN_BACK_TRAVERSE)) {
            return false;
        }

        int openStatementBracketIndex = path.position();
        int caretIndex = path.indexOfNextSignificantChar(BEGIN_BACK_TRAVERSE);
        if (caretIndex == -1) {
            return false;
        }
        int openBracketIndex = path.indexOfNextSignificantChar(caretIndex, OPEN_PARENTHESIS);
        if (openBracketIndex == -1) {
            return false;
        }
        int closeBracketIndex = path.indexOfClosingBracket(openBracketIndex, false, false);
        ...
        jpList.add(new JPToken(TokenType.BACK_TRAVERS,criteria));

        path.setPosition(closeStatementBracketIndex + 1);

        return path.currentIsTail() || readNextToken(appender, jpList);
    }

    //
    // . and ..
    //
    private boolean readDotToken(PathTokenAppender appender, LinkedList<JPToken> jpList) {

        if (path.currentCharIs(PERIOD) && path.nextCharIs(PERIOD)) {
            jpList.add(new JPToken(TokenType.DOT,PERIOD+""));
        ...
        return readNextToken(appender, jpList);
    }

    //
    // fooBar or fooBar()
    //
    private boolean readPropertyOrFunctionToken(PathTokenAppender appender, LinkedList<JPToken> jpList) {
        int tokenStartIndex = path.position();
        ...
        if(isFunction){
            jpList.add(new JPToken(TokenType.FUNCTION,path.subSequence(tokenStartIndex,path.position()).toString()));
            appender.appendPathToken(PathTokenFactory.createFunctionPathToken(property, functionParameters));
        } else {
            jpList.add(new JPToken(TokenType.PROPERTY,path.subSequence(tokenStartIndex,path.position()).toString()));
    ...
    // [?], [?,?, ..]
    //
    private boolean readPlaceholderToken(PathTokenAppender appender, LinkedList<JPToken> jpList) {
        int tokenStartIndex = path.position();
        ...
        jpList.add(new JPToken(TokenType.FILTER,path.subSequence(tokenStartIndex,expressionEndIndex+1).toString()));
    ...
    // [?(...)]
    //
    private boolean readFilterToken(PathTokenAppender appender, LinkedList<JPToken> jpList) {
        int tokenStartIndex = path.position();
        ...
        jpList.add(new JPToken(TokenType.FILTER,path.subSequence(tokenStartIndex,closeStatementBracketIndex+1).toString()));
    ...
    // *
    //
    private boolean readWildCardToken(PathTokenAppender appender, LinkedList<JPToken> jpList) {
        int tokenStartIndex = path.position();
        ...
        jpList.add(new JPToken(TokenType.WILDCARD,path.subSequence(tokenStartIndex,path.position()).toString()));
    ...
    // [1], [1,2, n], [1:], [1:2], [:2]
    //
    private boolean readArrayToken(PathTokenAppender appender, LinkedList<JPToken> jpList) {
        int tokenStartIndex = path.position();
        if (!path.currentCharIs(OPEN_SQUARE_BRACKET)) {
            return false;
        }
        ...
        if (isSliceOperation) {
            ...
            jpList.add(new JPToken(TokenType.SLICE_ARRAY,path.subSequence(tokenStartIndex,expressionEndIndex+1).toString()));
            ...
            jpList.add(new JPToken(TokenType.INDEX_ARRAY,path.subSequence(tokenStartIndex,expressionEndIndex+1).toString()));
    ...
    // ['foo']
    //
    private boolean readBracketPropertyToken(PathTokenAppender appender, LinkedList<JPToken> jpList) {
        int tokenStartIndex = path.position();
        if (!path.currentCharIs(OPEN_SQUARE_BRACKET)) {
            return false;
        }
        ...
        jpList.add(new JPToken(TokenType.PROPERTY,path.subSequence(tokenStartIndex,endBracketIndex).toString()));
